...
The server accepts `ALDUS_ADDR` (default `:8080`) and `ALDUS_DATA_DIR` (default `/data`).
...
`make docker` builds `ghcr.io/mahcks/aldus`. Run it with `docker compose up --build`, then open <http://localhost:8080>; persistent data is mounted at `./data`.
