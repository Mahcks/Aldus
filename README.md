...
Compose does not build Aldus or require the repository. It pulls the exact `ghcr.io/mahcks/aldus:0.1.0-beta.19` image and adds the restart policy, persistent volumes, health check, and safe localhost port mapping that a long `docker run` command would need. When `docker compose ps` reports **healthy**, open [http://localhost:8080](http://localhost:8080) and create the first account — it becomes the administrator. Do that before exposing Aldus to another machine.

> **Current beta note:** `0.1.0-beta.19` includes CPU alignment and publishes an optional NVIDIA CUDA image. Image downloads are large, so first startup time depends on your connection and host.
...
```sh
curl -fL https://github.com/Mahcks/Aldus/releases/download/v0.1.0-beta.19/compose.gpu.yml -o compose.gpu.yml
