...
export default function WorkScreen() {
  ...
  async function load() {
    ...
    try {
      ...
      const revisions = (
        await Promise.all(
          nextRepresentations.map(async (representation) =>
  ...
  const consume = (mode: 'read' | 'listen') =>
        ...
        <Section title="Choose your edition">
          <View style={shared.split}>
            <ChoiceList
              ...
              onSelect={setEPUBID}
            />
            <ChoiceList title="Narration" items={audio} selected={audioID} onSelect={setAudioID} />
          </View>
        </Section>
      ) : null}
      {canEdit && managing ? (
        <View style={styles.management}>
            ...
            {representations.length === 0 ? (
              ...
              representations.map((item) => {
                ...
                return (
                  ...
                  >
                    <Text style={shared.itemTitle}>{item.label}</Text>
          ...
          <Section title="Alignment management">
            <Text style={shared.itemMeta}>
              Choose exact revisions to prepare synchronized switching.
            </Text>
            <View style={shared.split}>
              <ChoiceList
                title="EPUB revision"
            ...
            {jobs.length === 0 ? (
              ...
              jobs.map((job) => (
                <View key={job.id} style={shared.listItem}>
                  <Text style={shared.itemTitle}>Alignment {job.state}</Text>
                  <Text style={shared.itemMeta}>
                    {new Date(job.created_at).toLocaleString()} · EPUB {job.epub_media_id} · audio{' '}
                    {job.audio_media_id}
                    {job.error ? ` · ${job.error}` : ''}
                  </Text>
                  <Text style={shared.mono}>
                    job {job.id}
                    {job.alignment_id ? ` · alignment ${job.alignment_id}` : ''}
                  </Text>
                  {!terminal.has(job.state) ? (
                    <Button
                      ...
                      onPress={async () => {
                        try {
                          await api.cancelAlignment(job.id);
                          const update = await api.alignmentJob(job.id);
                          setJobs((current) =>
...
const styles = StyleSheet.create({
  ...
  summary: { flex: 1, minWidth: 250, alignItems: 'flex-start', gap: 9 },
  workTitle: { maxWidth: 680, color: colors.ink, fontSize: 30, lineHeight: 36, fontWeight: '800' },
  author: { color: colors.muted, fontSize: 17 },
  progress: { color: colors.ink, fontSize: 14, fontWeight: '700', marginTop: 7 },
  status: { color: colors.accent, fontSize: 14, fontWeight: '700' },
  management: { paddingTop: 22, borderTopWidth: 1, borderTopColor: colors.line, gap: 26 },
});
