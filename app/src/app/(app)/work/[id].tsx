...
export default function WorkScreen() {
  ...
  const [kind, setKind] = useState('epub'); const [label, setLabel] = useState(''); const [title, setTitle] = useState(''); const [author, setAuthor] = useState(''); const [epubID, setEPUBID] = useState(''); const [audioID, setAudioID] = useState(''); const [jobs, setJobs] = useState<AlignmentJob[]>([]);
  async function load() {
  ...
  return <Page title={work.title} back={<Button label="Library" kind="quiet" onPress={() => router.back()} />}>
    {error ? <Notice danger>{error}</Notice> : null}
    <View style={styles.hero}>
      <View style={styles.cover}><Text style={styles.coverLetter}>{work.title.slice(0, 1).toUpperCase()}</Text></View>
      <View style={styles.summary}><Text style={styles.workTitle}>{work.title}</Text><Text style={styles.author}>{work.author || 'Unknown author'}</Text><Text style={styles.status}>{syncLabel}</Text><Row><Button label="Read" kind="primary" disabled={!selectedEPUB} onPress={() => consume('read')} /><Button label="Listen" disabled={!selectedAudio} onPress={() => consume('listen')} /></Row></View>
    </View>
...
const styles = StyleSheet.create({
  ...
  cover: { width: 150, height: 205, backgroundColor: '#73402f', borderWidth: 1, borderColor: '#5f3326', alignItems: 'center', justifyContent: 'center' }, coverLetter: { color: '#f8eee3', fontSize: 58, fontWeight: '800' },
  summary: { flex: 1, minWidth: 250, alignItems: 'flex-start', gap: 9 }, workTitle: { maxWidth: 680, color: colors.ink, fontSize: 30, lineHeight: 36, fontWeight: '800' }, author: { color: colors.muted, fontSize: 17 }, status: { color: colors.accent, fontSize: 14, fontWeight: '700', marginVertical: 6 },
});
