...
export default function Login() {
  ...
  if (auth.error) {
    return (
      <AuthLayout>
        <ErrorState title="Couldn’t open your library">{auth.error}</ErrorState>
        <Button label="Retry connection" kind="primary" onPress={auth.refresh} />
        {Platform.OS !== 'web' ? (
          <Button label="Change server" kind="secondary" onPress={() => router.push('/connect')} />
        ) : null}
      </AuthLayout>
    );
  }
