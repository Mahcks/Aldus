...
export type ReaderLocation = {
  href: string;
  cfi: string;
  sync?: { href: string; locator: { type: 'dom-element'; dom_path: string; segment_id: string }; offset: number };
...
type Props = { source?: string | Blob; product?: boolean; segments?: SyncSegment[]; onLocation?: (location: ReaderLocation) => void; onReady?: () => void };
type SyncSegment = { id: string; ordinal: number; epub_href: string; epub_locator: unknown; highlightable: boolean };

export const EPUBReader = forwardRef<EPUBReaderHandle, Props>(function EPUBReader({ source = '/media/alice.epub', product, segments = [], onLocation, onReady }, ref) {
  const host = useRef<View>(null);
  const reader = useRef<any>(null);
  const selection = useRef<{ index: number; range: Range } | undefined>(undefined);
  const cursor = useRef<ReaderLocation>(undefined); const page = useRef<{ href: string; cfi: string; range: Range; state: ReaderLocation['syncState'] }>(undefined);
  ...
  useImperativeHandle(ref, () => ({
    ...
    async restoreLocation(value, highlight = false) {
      ...
      const location = value as { href?: string; cfi?: string; offset?: number; locator?: { type?: string; dom_path?: string; segment_id?: string; start?: RangeBoundary; end?: RangeBoundary } };
      if (location.cfi) {
        await view.goTo(location.cfi);
        return true;
      }
      if (!location.href || location.locator?.type !== 'dom-element' || !location.locator.dom_path) return false;
      if (location.locator.segment_id) cursor.current = { href: location.href, cfi: '', sync: { href: location.href, locator: { type: 'dom-element', dom_path: location.locator.dom_path, segment_id: location.locator.segment_id }, offset: location.offset ?? 0 }, reason: 'restore' };
      ...
      if (location.locator.start && location.locator.end) {
        range.setStart(resolveDOMPath(content.doc, location.locator.start.dom_path), location.locator.start.node_offset);
        range.setEnd(resolveDOMPath(content.doc, location.locator.end.dom_path), location.locator.end.node_offset);
      ...
      if (cursor.current) onLocationRef.current?.({ ...cursor.current, cfi, syncState: page.current?.state, reason: 'restore' });
      return true;
    },
  }), []);
    ...
    import('foliate-js/view.js').then(async () => {
      ...
      view.addEventListener('load', ({ detail: { doc, index } }: CustomEvent) => {
        ...
        doc.addEventListener('click', (event: MouseEvent) => {
          ...
          if (!match) return onLocationRef.current?.({ href, cfi: current.cfi, syncState: current.state, reason: 'explicit' });
          cursor.current = syncLocation(href, current.cfi, match, current.state, 'explicit');
          markWord(point);
          if (__DEV__) console.debug('Aldus reading cursor', { reason: 'explicit', href, boundary: boundary(point), segment_id: match.id, offset: match.offset });
      ...
      view.addEventListener('relocate', ({ detail }: CustomEvent) => {
        ...
        cursor.current = relocatedCursor(cursor.current, fallback ? syncLocation(href, detail.cfi, fallback, state, 'relocate') : undefined);
        const location: ReaderLocation = cursor.current ? { ...cursor.current, cfi: detail.cfi, syncState: state, reason: 'relocate' } : { href, cfi: detail.cfi, syncState: state, reason: 'relocate' };
        if (__DEV__) console.debug('Aldus relocation', { href, visible_start: boundary(range), visible_end: boundary(range, true), direction: direction.current, cursor: cursor.current?.sync, candidate_segments: intersectingSegments(range, href, segments).map((item) => item.id), sync_state: state, listen: cursor.current?.sync ? 'enabled: valid cursor' : state === 'partial' ? 'disabled: move to aligned text' : 'disabled: no valid cursor' });
        onLocationRef.current?.(location);
      });
      await view.open(source instanceof Blob ? new File([source], 'book.epub', { type: 'application/epub+zip' }) : source);
  ...
  return (
    <View style={styles.reader}>
      ...
      <View style={styles.navigation}>
        <Pressable accessibilityRole="button" style={styles.button} onPress={() => { direction.current = 'backward'; reader.current?.goLeft(); }}><Text style={styles.buttonText}>Previous page</Text></Pressable>
        ...
        <Pressable accessibilityRole="button" style={styles.button} onPress={() => { direction.current = 'forward'; const current = page.current; if (current) { const match = trailingSegment(current.range, current.href, segments); if (match) { cursor.current = syncLocation(current.href, current.cfi, match, current.state, 'forward'); onLocationRef.current?.(cursor.current); } } reader.current?.goRight(); }}><Text style={styles.buttonText}>Next page</Text></Pressable>
      </View>
    </View>
  );
});
...
function containingSegment(point: Range, href: string, segments: SyncSegment[]) {
  for (const segment of segments) {
    const locator = segment.epub_locator as { dom_path?: string; start?: RangeBoundary; end?: RangeBoundary };
    if (!segment.highlightable || segment.epub_href !== href || !locator?.dom_path || !locator.start || !locator.end) continue;
    try {
      const doc = point.startContainer.ownerDocument!;
      const range = segmentRange(doc, locator);
      if (range.comparePoint(point.startContainer, point.startOffset) !== 0 || sameBoundary(point, locator.end, doc)) continue;
      const before = range.cloneRange(); before.setEnd(point.startContainer, point.startOffset);
      const total = normalize(range.toString()).length;
      return { id: segment.id, domPath: locator.dom_path, offset: total ? Math.min(1_000_000, Math.round(normalize(before.toString()).length * 1_000_000 / total)) : 0 };
    } catch { /* malformed boundaries fail closed */ }
  }
}
...
function intersectingSegments(visible: Range, href: string, segments: SyncSegment[]) {
  return segments.filter((segment) => {
    ...
    try {
      ...
      const range = segmentRange(doc, locator);
      return comparePoints(range.startContainer, range.startOffset, visible.endContainer, visible.endOffset, doc) < 0
        && comparePoints(range.endContainer, range.endOffset, visible.startContainer, visible.startOffset, doc) > 0;
    } catch { return false; }
  });
}

function pageSyncState(visible: Range, href: string, segments: SyncSegment[]): ReaderLocation['syncState'] {
  const candidates = intersectingSegments(visible, href, segments);
  return classifyPageSync(candidates.filter((segment) => segment.highlightable).length, candidates.filter((segment) => !segment.highlightable).length);
}

function trailingSegment(visible: Range, href: string, segments: SyncSegment[]) {
  const candidates = intersectingSegments(visible, href, segments).filter((segment) => segment.highlightable).sort((a, b) => a.ordinal - b.ordinal);
  ...
  try {
    const doc = visible.startContainer.ownerDocument!;
    const range = segmentRange(doc, locator);
    if (range.comparePoint(visible.endContainer, visible.endOffset) === 0 && !sameBoundary(visible, locator.end, doc, true)) {
      const point = visible.cloneRange(); point.collapse(false);
...
function syncLocation(href: string, cfi: string, match: { id: string; domPath: string; offset: number }, syncState: ReaderLocation['syncState'], reason: ReaderLocation['reason']): ReaderLocation {
  return { href, cfi, sync: { href, locator: { type: 'dom-element', dom_path: match.domPath, segment_id: match.id }, offset: match.offset }, syncState, reason };
}
...
function comparePoints(aNode: Node, aOffset: number, bNode: Node, bOffset: number, doc: Document) {
  const a = doc.createRange(); a.setStart(aNode, aOffset); a.collapse(true);
  const b = doc.createRange(); b.setStart(bNode, bOffset); b.collapse(true);
...
function sameBoundary(visible: Range, boundary: RangeBoundary, doc: Document, end = false) {
  const point = doc.createRange(); point.setStart(resolveDOMPath(doc, boundary.dom_path), boundary.node_offset); point.collapse(true);
  const current = doc.createRange(); current.setStart(end ? visible.endContainer : visible.startContainer, end ? visible.endOffset : visible.startOffset); current.collapse(true);
  return current.compareBoundaryPoints(Range.START_TO_START, point) === 0;
}

function caretAt(doc: Document, x: number, y: number) {
  const modern = (doc as Document & { caretPositionFromPoint?: (x: number, y: number) => { offsetNode: Node; offset: number } | null }).caretPositionFromPoint?.(x, y);
  const legacy = (doc as Document & { caretRangeFromPoint?: (x: number, y: number) => Range | null }).caretRangeFromPoint?.(x, y);
  if (legacy) return legacy;
  if (!modern) return null;
  const range = doc.createRange(); range.setStart(modern.offsetNode, modern.offset); range.collapse(true); return range;
}
...
function markWord(point: Range) {
  ...
  let start = point.startOffset; let end = start;
  while (start > 0 && /[\p{L}\p{N}'’]/u.test(text[start - 1])) start--;
  while (end < text.length && /[\p{L}\p{N}'’]/u.test(text[end])) end++;
  if (start === end) return;
  const range = point.cloneRange(); range.setStart(point.startContainer, start); range.setEnd(point.startContainer, end);
  const selected = point.startContainer.ownerDocument?.getSelection(); selected?.removeAllRanges(); selected?.addRange(range);
}

function boundary(range: Range, end = false) {
  const node = end ? range.endContainer : range.startContainer; const offset = end ? range.endOffset : range.startOffset;
...
const styles = StyleSheet.create({
  reader: { flex: 1, minHeight: 560 }, book: { flex: 1, minHeight: 500, overflow: 'hidden', backgroundColor: '#fffdf8' },
  navigation: { minHeight: 52, borderTopWidth: 1, borderTopColor: '#c9c0b3', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 },
