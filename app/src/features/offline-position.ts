...
export function offlineAudioToCanonical(
  ...
  const segment = alignment.segments.find(
    (item) =>
      item.highlightable &&
      item.audio_resource === locator.resource &&
      item.audio_start_ms <= locator.timestamp_ms &&
      item.audio_end_ms > locator.timestamp_ms,
  );
  if (!segment || segment.audio_end_ms <= segment.audio_start_ms) return;
  const exactOffset = wordOffset(segment.text, segment.word_timings, locator.timestamp_ms);
  return {
