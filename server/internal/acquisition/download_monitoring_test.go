...
func TestDownloadMonitoringClassifiesOnlyDurableFailures(t *testing.T) {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM pragma_table_info('acquisition_requests') WHERE name='download_job_id'`).Scan(&monitoringColumns); err != nil {
		t.Fatal(err)
	}
	...
	failed, err := store.monitorDownload(ctx, downloadMonitorRequest{id: "complete", progressUpdated: recent}, &Download{JobID: "jobID", State: "uploading", Progress: 1}, now)
	if err != nil || failed {
		t.Fatalf("completed monitor failed=%v err=%v", failed, err)
	}

	var jobID, seen string
	var progress float64
	if err := db.QueryRow(`SELECT download_job_id,download_last_seen_at,download_progress FROM acquisition_requests WHERE id='complete'`).Scan(&jobID, &seen, &progress); err != nil || jobID != "jobID" || seen == "" || progress != 1 {
		t.Fatalf("completed monitoring = %q %q %v, %v", jobID, seen, progress, err)
	}

	failed, err = store.monitorDownload(ctx, downloadMonitorRequest{id: "error", progressUpdated: recent}, &Download{JobID: "bad", State: "missingFiles", Progress: .4}, now)
	if err != nil || !failed {
	...
	failed, err = store.monitorDownload(ctx, downloadMonitorRequest{id: "metadata", progressUpdated: metadataOld}, &Download{JobID: "dead", State: "forcedMetaDL"}, now)
	if err != nil || !failed {
	...
	failed, err = store.monitorDownload(
		ctx,
		downloadMonitorRequest{id: "stalled", progress: .3, progressUpdated: old},
		&Download{JobID: "stall", State: "stalledDL", Progress: .3},
		now,
	)
	if err != nil || !failed {
	...
	failed, err = store.monitorDownload(
		ctx,
		downloadMonitorRequest{id: "paused", progress: .3, progressUpdated: old},
		&Download{JobID: "pause", State: "pausedDL", Progress: .3},
		now,
	)
	if err != nil || failed {
	...
	failed, err = store.monitorDownload(
		ctx,
		downloadMonitorRequest{id: "resumed", progress: .3, progressUpdated: old},
		&Download{JobID: "resume", State: "downloading", Progress: .4},
		now,
	)
	if err != nil || failed {
