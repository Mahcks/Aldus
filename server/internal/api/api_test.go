...
func TestHandler(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			Handler(fs.FS(web), nil, position.New(db), authStore, catalog.New(db), koreader.Credentials{User: "aldus", Key: "aldus"}).ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, test.target, nil))
	...
	recorder := httptest.NewRecorder()
	Handler(fs.FS(web), nil, position.New(db), authStore, catalog.New(db), koreader.Credentials{User: "aldus", Key: "aldus"}).ServeHTTP(recorder, httptest.NewRequest(http.MethodOptions, "/api/alignments/fixture-alignment/progress", nil))
...
func TestMediaSupportsCrossOriginRanges(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	Handler(nil, media, position.New(db), authStore, catalog.New(db), koreader.Credentials{}).ServeHTTP(recorder, request)
