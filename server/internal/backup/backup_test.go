...
func TestCreateVerifyAndRestore(t *testing.T) {
	...
	if _, err := db.Exec(
		`INSERT INTO acquisition_settings(id,indexer_url,indexer_api_key,qbittorrent_url,qbittorrent_username,qbittorrent_password,qbittorrent_category,updated_at) VALUES(1,?,?,'http://qbittorrent','aldus',?,'aldus','2026-01-01')`,
		"http://user:"+urlSecret+"@prowlarr",
		indexerSecret,
		qbitSecret,
	); err != nil {
		t.Fatal(err)
	...
	if _, err := db.Exec(`
        UPDATE acquisition_settings SET sabnzbd_url='http://sab', sabnzbd_api_key='sab-private-key';
        INSERT INTO acquisition_download_clients(id,kind,url,password)
        VALUES('sab','sabnzbd','http://sab','sab-private-key');
        UPDATE acquisition_results SET release_metadata='{"guid":"https://indexer/?apikey=private"}';
        UPDATE acquisition_requests SET selected_release_metadata='{"guid":"https://indexer/?apikey=private"}';
    `); err != nil {
		t.Fatal(err)
	}
	...
	if err := restoredDB.QueryRow(`
        SELECT
            (SELECT count(*) FROM acquisition_settings WHERE sabnzbd_api_key!='') +
            (SELECT count(*) FROM acquisition_download_clients WHERE password!='') +
            (SELECT count(*) FROM acquisition_results WHERE release_metadata!='{}') +
            (SELECT count(*) FROM acquisition_requests WHERE selected_release_metadata!='{}')
    `).Scan(&remainingSecrets); err != nil || remainingSecrets != 0 {
		t.Fatalf("restored provider secrets=%d err=%v", remainingSecrets, err)
	}
