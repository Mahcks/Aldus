...
func TestMigrationCreatesAndReopensCurrentVersion(t *testing.T) {
	...
	if version := schemaVersion(t, db); version != 34 {
	...
	if version := schemaVersion(t, db); version != 34 {
		t.Fatalf("schema version after reopen = %d, want 34", version)
	}
}
...
func TestMigrationRejectsNewerDatabase(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "schema version 35 is newer than supported version 34") {
...
func TestMigrationRollsBackFailedVersion(t *testing.T) {
	...
	if version := schemaVersion(t, db); version != 34 {
		t.Fatalf("schema version after rollback = %d, want 34", version)
	}
	var exists int
