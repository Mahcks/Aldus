...
func TestMigrationCreatesAndReopensCurrentVersion(t *testing.T) {
	...
	if version := schemaVersion(t, db); version != 24 {
	...
	if version := schemaVersion(t, db); version != 24 {
		t.Fatalf("schema version after reopen = %d, want 24", version)
	}
}
...
func TestMigrationRejectsNewerDatabase(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "schema version 25 is newer than supported version 24") {
...
func TestMigrationRollsBackFailedVersion(t *testing.T) {
	...
	if version := schemaVersion(t, db); version != 24 {
		t.Fatalf("schema version after rollback = %d, want 24", version)
	}
	var exists int
