...
func TestMigrationCreatesAndReopensCurrentVersion(t *testing.T) {
	...
	if version := schemaVersion(t, db); version != 3 {
	...
	defer db.Close()
	if version := schemaVersion(t, db); version != 3 {
		t.Fatalf("schema version after reopen = %d, want 3", version)
	}
}
...
func TestMigrationRejectsNewerDatabase(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "schema version 4 is newer than supported version 3") {
